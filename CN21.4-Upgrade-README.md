...
- and to update RHOSP16.1 with CN2011 to RHOSP16.2 with CN21.4
...
In case of FFU upgrade RHOSP13 => RHOSP16 the difference in the below procedure is  [TODO]

## Prerequisites:

1. New Contrail Control plane is deployed on K8S/OpenShift cluster

- It should use self-signed CA prepared before the deployment
In case of RHOSP uses IPA for certificates management cluster should use
CA bundle with own CA and IPA CA, e.g.:
```bash
# assuming that k8s cluster ca is in ca.crt.pem
cat ca.crt.pem /etc/ipa/ca.crt  > ca-bundle.pem
...
Follow instruction [tools/contrail/issu/README.md] from 1 to 9
...
Follow instruction [tools/contrail/issu/README.md] from 10 to 15
