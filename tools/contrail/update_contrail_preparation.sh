...
echo "INFO: CONTRAIL_NEW_IMAGE_TAG=$CONTRAIL_NEW_IMAGE_TAG"
...
for ip_addr in $(openstack server list -f value -c Networks | cut -d '=' -f 2); do
    ...
    scp ${SSH_OPTIONS} $my_dir/pull_new_contrail_images.sh ${SSH_USER}@${ip_addr}:
    ssh ${SSH_OPTIONS} ${SSH_USER}@${ip_addr} CONTRAIL_IMAGE_PREFIX=${CONTRAIL_IMAGE_PREFIX} CONTRAIL_NEW_IMAGE_TAG=${CONTRAIL_NEW_IMAGE_TAG} ./pull_new_contrail_images.sh
done
...
for ip_addr in $(openstack server list -f value -c Networks | cut -d '=' -f 2); do
    ...
    scp ${SSH_OPTIONS} $my_dir/stop_contrail_api_containers.sh ${SSH_USER}@${ip_addr}:
    ssh ${SSH_OPTIONS} ${SSH_USER}@${ip_addr} STOP_CONTAINERS="${STOP_CONTAINERS_ESC}" ./stop_contrail_api_containers.sh
done
